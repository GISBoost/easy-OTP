// Converts every per-origin *.geojson in ../data/<variant>/ into a geobuf
// *.pbf binary of the same basename, then deletes the source .geojson.
...
//
// Usage: node convert.js <variant: static|rt>
const fs = require("fs");
...
if (!["static", "rt"].includes(variant)) {
  console.error("Usage: node convert.js <variant: static|rt>");
  process.exit(1);
}

const dir = path.join(__dirname, "..", "data", variant);
...
console.log(`${variant}: converted ${files.length} files, total .pbf size ${(totalOut / 1e6).toFixed(1)} MB`);
