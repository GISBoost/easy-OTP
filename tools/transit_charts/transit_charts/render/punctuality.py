...
def dot_and_whisker(
    ...
    *,
    ...
    a convincing-looking line. The function takes a single route and picks the busiest
    direction unless told otherwise, reporting which.
    """
    ...
    return style.save(
        fig, stats, style.chart_params("C9", source, len(table),
                            {"route": route, "direction": direction, "headsign": headsign,
                             "min_n": min_n,
                             "min_trip_coverage": min_trip_coverage}, notes),
        out_prefix,
...
def percentile_fan(
    ...
    *,
    ...
    for ax, key in zip(axes, keys):
        ...
        ax.fill_between(solid.bucket, solid.p25_min, solid.p75_min, color=colours[key],
                        alpha=0.32, linewidth=0, label="p25-p75")
        ax.plot(solid.bucket, solid.p50_min, color=colours[key], linewidth=1.8, label="median")
        ax.axhline(0, color="black", linewidth=0.8, alpha=0.5)
        ax.set_ylabel(f"route {key}\ndelay (min)")
        ax.grid(True, **style.GRID_KW)
        style.mark_thin_buckets(ax, part.bucket.tolist(), part.below_min_n.tolist())
    ...
    return style.save(
        fig, stats, style.chart_params("C10", source, len(table),
                            {"routes": keys, "bucket_minutes": bucket_minutes,
                             "min_n": min_n}, notes),
...
def punctuality_bands(
    ...
    *combine* adds a pooled panel above the per-route ones, keyed `ALL` in the sidecar. Pooled
    ...
    panels = [(key, subset[subset.route_short_name == key]) for key in keys]
    ...
    if combine:
        panels.insert(0, (POOLED_KEY, subset))
    fig, axes = _facet([key for key, _part in panels], height_each=2.4)

    parts = []
    for ax, (key, part) in zip(axes, panels):
        ...
        ax.stackplot(
            shares.index,
            [shares[name] for name in band_names],
            labels=band_names,
        ...
        ax.set_ylim(0, 1)
        ax.set_ylabel(
            f"all {len(keys)} routes\nshare" if key == POOLED_KEY else f"route {key}\nshare"
        )
        ax.grid(True, **style.GRID_KW)
        style.mark_thin_buckets(ax, list(shares.index), list(totals < min_n))
    ...
    style.caption(axes[-1], notes)
    return style.save(
        fig, out, style.chart_params("C11", source, len(table),
                          {"routes": keys, "bucket_minutes": bucket_minutes,
                           "min_n": min_n, "combine": combine,
                           "bands_seconds": [[n, lo, hi] for n, lo, hi, _c in bands]}, notes),
        out_prefix, rect=(0, 0.06, 1, 0.92),
    )
