...
def citywide_hourly(
    ...
    """One row per time-of-day bucket, pooling every stop's pooled headways city-wide.

    The chart's input - fluctuation over the day, not the cumulative per-stop figure the map
    shows. Bucketed on the LATER vehicle's arrival time (obs_local), same convention as every
    other time-of-day chart in this tool (`tidy.local_time_bucket`).
    """
    crossings = _pooled_crossings(frame, outages)
